...
try {
    ...
    // First, fetch the associated user ID from the staffs table
    $fetchUserIdSql = "SELECT user_id FROM staffs WHERE id = ?";
    ...
    // Second, delete the staffs record
    $staffsDeleteSql = "DELETE FROM staffs WHERE id = ?";
    $staffsStmt = $conn->prepare($staffsDeleteSql);
    $staffsStmt->bind_param("i", $dataId);
    ...
    $staffsDeleteSuccess = $staffsStmt->execute();
    $userDeleteSuccess = $userStmt->execute();

    if ($staffsDeleteSuccess && $userDeleteSuccess) {
